...
if [ ! -f "${latest_json}" ]; then
    curl -LJSs https://api.github.com/repos/ryanoasis/nerd-fonts/releases/latest >"${latest_json}"
fi

tagName=$(jq -r '.tag_name' "${latest_json}")

options=$(jq -r '.assets[] | "\(.name) \(.browser_download_url)"' "${latest_json}" | while IFS=' ' read -r assetName url; do
    case "${assetName}" in
    *.tar.xz)
