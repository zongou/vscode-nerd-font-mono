...
latest_json=$($MD_EXE getLatest -- ryanoasis/nerd-fonts)
tagName=$(printf '%s' "${latest_json}" | jq -r '.tag_name')

options=$(printf '%s' "${latest_json}" | jq -r '.assets[] | "\(.name) \(.browser_download_url)"' | while IFS=' ' read -r assetName url; do
    case "${assetName}" in
    *.tar.xz)
